library(methods)
library(mxnet)
setwd("C:/Users/Po/Sync/Fbook RNN")
...
#############################
#Data: Get training data and eval data
###############################
#download.data("./data/")
#ret <- make.data("./data/input.txt", seq.len=seq.len)
ret <- make.data("Alice_clean.txt", seq.len=seq.len) #save(ret, file = "AliceRet.rda")
X <- ret$data
dic <- ret$dic
...
#Inference
#helper functions for random sample
